...
| 热键 | 默认 | 说明 |
|------|------|------|
| 切换录制 | `Right Shift`（单击） | 第一次点击开始录制，第二次点击停止录制 |
| 取消录制 | `Right Shift + C` | 停止录制并丢弃音频，不进行后续处理 |

Right Shift 热键区分单击（切换录制）和组合键（按住它加其他键不触发录制）。
...
3. 将光标放在需要输入的位置
4. 按 `Right Shift`（单击一次）开始录制（状态气泡显示"录制中..."）
5. 说话完毕后，按 `Right Shift`（单击一次）停止录制
6. 等待处理完成（状态气泡显示"润色中..."），润色后的文本将自动出现在光标位置
7. 如需放弃本次录制，按 `Right Shift + C` 取消（音频将被丢弃）
