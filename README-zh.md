...
- **语音命令**: 说出配置的命令短语（如"换行"、"撤销"、"取消"）即执行对应动作，不再润色粘贴；短语可在设置"语音命令"标签页编辑
- **场景预设（按应用切换）**: 规则将前台进程名（如 Code.exe）映射到已保存配置——终端里免润色直出、聊天软件里保留润色，按录音生效且不改动当前配置档案
- **快捷键手势**: 右 Alt 支持可选手势——双击进入免润色录音，长按按住说话（快捷键标签页可配置）
- **使用统计**: 历史窗口顶部汇总条显示总条数、总字数、今日条数与估算节省时间
- **录音存档**: 可选将每次录音的 WAV 保存到 `~/.voice-type/audio-archive/` 并按保留时长自动清理；存档条目可在历史对话框中回放
- **实时字幕窗口**: 独立的可缩放置顶字幕卡同步显示流式识别文本并支持复制（托盘 →"字幕窗口..."）；覆盖层字幕面板可在设置中开关
